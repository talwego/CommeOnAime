...
contientLogin(control: AbstractControl): ValidationErrors | null {
  ...
  return password?.value.toString().includes(control.get('login')?.value.toString())
    ? { contientLogin: true }
    : null;
}

passwordAndConfirmationEquals(control: AbstractControl): ValidationErrors | null {
...
save() {let user = { login: this.form.get('groupeInfo.login')?.value, password: this.form.get('groupeInfo.groupePassword.password')?.value};
  if (
    this.form.get('compteVIP')?.value || this.form.get('age')?.value || this.form.get('genre')?.value || this.form.get('taille')?.value || this.form.get('poids')?.value || this.form.get('vegetarien')?.value || this.form.get('vegan')?.value || this.form.get('activitePhysique')?.value){
    Object.assign(user, {
    ...
    console.log(user)
  this.userService.create(user).subscribe((data) => {this.router.navigateByUrl('/home');});
}
}
}
